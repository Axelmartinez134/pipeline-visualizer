...
export class Camera {
  ...
  animateToProcess(processId, duration = 2) {
    try {
      // ========================================
      // AUDIT PAGE MODIFICATION: LOCK CAMERA AT OVERVIEW
      // ========================================
      // For the simplified audit demo, the camera must NEVER move from overview
      // This prevents any zoom-in behavior and keeps the entire pipeline visible
      if (processId !== 'overview') {
        console.log('[Audit] Camera locked to overview, ignoring transition to:', processId);
        return false; // Reject all non-overview transitions
      }
      // ========================================
