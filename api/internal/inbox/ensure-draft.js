...
module.exports = async function handler(req, res) {
  ...
  // Phase 5: support first_message + reply. (followup is scaffolded later)
  ...
  // Return existing pending draft if present (one-per-lead invariant).
  ...
  if (existing) return json(res, 200, { ok: true, draft: existing, reused: true });

  // Load lead + profile context (apify is optional for inbox drafting).
  ...
  const { data: inserted, error: insErr } = await supabaseAdmin
    ...
    .insert({
      ...
      ai_prompt: user,
      ai_context: { ...context, prompt_source: promptSource },
      ai_request_json: {
        provider: 'anthropic',
  ...
  return json(res, 200, { ok: true, draft: inserted, reused: false });
};
